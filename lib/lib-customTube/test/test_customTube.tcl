...
namespace eval control {
    ...
    #
    # -- MODEL --
proc model::init {centerLineDef profileDef} {
        #
    variable tubeDictionary
        #
    variable segmentDictionary
    ...
    set tubeDictionary          [dict create]
        #
        #
    create_SegmentDictionary    $centerLineDef
        #
    create_ProfileDictionary    $profileDef
        #
    update_InsideLengths
        #
    update_SegmentOutline
        #
    update_ShapeDictionary
        #
    # update_ShapeDictionary_org
        #
    create_AnyDictionary
        #
    # create_DebugDictionary
        #

    # puts "\n --> \$centerlineDictionary"
    # appUtil::pdict $centerlineDictionary        
        #
    # puts "\n --> \$profileDictionary"
    # appUtil::pdict $profileDictionary        
        #
    # puts "\n --> \$segmentDictionary"
    # appUtil::pdict $segmentDictionary        
        #
    # puts "\n --> \$shapeDictionary"
    # appUtil::pdict $shapeDictionary        
        #
        #
    dict append tubeDictionary profile      $profileDictionary
    dict append tubeDictionary segment      $segmentDictionary
    dict append tubeDictionary shape        $shapeDictionary
    dict append tubeDictionary any          $anyDictionary
    # dict append tubeDictionary debug        $debugDictionary
        #
        #
    # puts "\n --> \$tubeDictionary"
...
proc model::getProfileProperty {length} {
    ...
    variable profileDictionary
        #
    # appUtil::pdict $profileDictionary 1 "        "
        #
    set indexList       [lsort -real [dict keys $profileDictionary]]
    set x_Start         [lindex $indexList 0]
    set x_End           [lindex $indexList end]
    set indexList       [lrange $indexList 1 end]
        #
    set dictSegement    [dict get $profileDictionary $x_End]
    ...
    foreach x_End $indexList {
        ...
        if {$x_Start <= $length && $length < $x_End} {
            ...
            set dictSegement    [dict get $profileDictionary $x_Start]
...
proc model::create_SegmentDictionary {centerLineDef} {
        #
    # puts "\n    model::create_SegmentDictionary $centerLineDef"
        #
    variable segmentDictionary
    set segmentDictionary    [dict create]
        #
    set myLength               0
    set myPosition          {0 0}
    set myDirVector         {1 0}
    ...
    set i   0    
        #
    foreach {type value} $centerLineDef {
        switch -exact $type {
            l {     # puts    "       line: $value"
                ...
                dict append segmentDictionary $i $lineDict
                incr i
            ...
            a {     # puts    "       arc:  $value"
                ...
                dict append segmentDictionary $i $arcDict
                incr i
        ...
        #
    # puts "\n    -> \$segmentDictionary"
    # appUtil::pdict $segmentDictionary 1 "        "    
        #
    return
        #
}
proc model::create_ProfileDictionary {profileDef} {
        #
    # puts "\n    model::create_ProfileDictionary $profileDef"
        #
    variable profileDictionary    
    set profileDictionary       [dict create]
            #
    ...
    foreach {x_extend y} $profileDefinition {
        ...
        foreach {profileDict x_last y_last} $retValue break
            # puts "       ----> $x_last   $y_last"
        dict append profileDictionary $myLength $profileDict
        set myLength    $x_last
    }
        # 
    # puts "\n    -> \$profileDictionary"
...
proc model::create_DebugDictionary {} {
    ...
    set debugDictionary         [dict create]
        #
    set dict_centerLine $segmentDictionary
    set dict_profile    $profileDictionary
    ...
    set kRight      {}
        #
        #
    foreach key [dict keys $segmentDictionary] {
        set keyDict     [dict get $segmentDictionary $key]
...
proc model::update_InsideLengths {} {
    ...
    # puts "\n -- <D> --\n"
        ...
        #
    set list_profile_x  [dict keys $profileDictionary]
        #
    ...
    foreach key [dict keys $segmentDictionary] {
        ...
        if {$def_Type eq {line}} {
            ...
            set lng_Inside  [lsort -unique -real $lng_Inside]
                #
            dict set    segmentDictionary $key inside   lengths [list $lng_Inside]
                #
        }
            # appUtil::pdict  $keyDict
    }
        ...
        switch -exact $def_Type {
            line {
                ...
                foreach lng $lengths {
                    ...
                    #
                dict set segmentDictionary $key shape left  [list [join $crv_Left  " "]]
                dict set segmentDictionary $key shape right [list [join $crv_Right " "]]
                    #
            }
            arc {
                    #
                ...
                set pos__StartRight [vectormath::addVector $pos_StartRight [vectormath::rotatePoint {0 0} [list 1 [expr -1.0 * $k]] $dir_Start]] 
                    ...
                    #
                dict set segmentDictionary $key start offset    $off_Start
                dict set segmentDictionary $key start k         $k
                dict set segmentDictionary $key start pos_Left  [list $pos_StartLeft]
                dict set segmentDictionary $key start pos_Right [list $pos_StartRight]
                    #
                    #
                    # puts "\n"
                    #
                ...
                # puts "                  \$pos_EndRight      $pos_EndRight"
                    #
                dict set segmentDictionary $key end   offset    $off_End
                dict set segmentDictionary $key end   k         $k
                dict set segmentDictionary $key end   pos_Left  [list $pos_EndLeft]
                dict set segmentDictionary $key end   pos_Right [list $pos_EndRight]
                    #
                    #
                ...
                set pos_TngtRight   [vectormath::intersectPoint $pos_StartRight $pos__StartRight $pos_EndRight $pos__EndRight   center]
                    ...
                    # puts ""
                    #
                dict set segmentDictionary $key inside pos_TangentLeft  [list $pos_TngtLeft]
                dict set segmentDictionary $key inside pos_TangentRight [list $pos_TngtRight]
                    #
                    #
                    # http://stackoverflow.com/questions/1734745/how-to-create-circle-with-b%C3%A9zier-curves
                    # for Bezier curve with n segments the optimal distance to the control points, 
                ...
                set l_control   [expr ($radius + $off_Start * $orient) * $f_control ]
                 set pos_StartRightControl   [vectormath::addVector $pos_StartRight  [vectormath::unifyVector $pos_StartRight $pos_TngtRight $l_control]]
                    ...
                    #
                dict set segmentDictionary $key start ctr_Factor        $f_control
                dict set segmentDictionary $key start pos_LeftCtrl      $pos_StartLeftControl
                dict set segmentDictionary $key start pos_RightCtrl     $pos_StartRightControl
                    #
                    # -- End
                ...
                set pos_EndRightControl     [vectormath::addVector $pos_EndRight    [vectormath::unifyVector $pos_EndRight $pos_TngtRight $l_control]]
                    #
                dict set segmentDictionary $key end   ctr_Factor        $f_control
                dict set segmentDictionary $key end   pos_LeftCtrl      $pos_EndLeftControl
                dict set segmentDictionary $key end   pos_RightCtrl     $pos_EndRightControl
                    #
                    #
                ...
                set crv_Right   [model::bezier::create [join "$pos_StartRight $pos_StartRightControl $pos_EndRightControl $pos_EndRight" " "]]
                    #
                dict set segmentDictionary $key shape left  [list $crv_Left]
                dict set segmentDictionary $key shape right [list $crv_Right]
                    #
            }
            default {}
        }
...
proc model::update_ShapeDictionary {} {
        #
    # puts "\n    model::update_ShapeDictionary"
        #
    variable segmentDictionary
    variable shapeDictionary
        #
    ...
    foreach {x y} $_shapeRight {
        ...
        #
    set shapeDictionary [dict create shapeLeft [list $shapeLeft] shapeRight [list $shapeRight]]
        #
        # puts "\n    -> \$shapeDictionary"
        # appUtil::pdict $shapeDictionary 1 "        "
        #
    return
        #
}
...
proc control::create_dictPath {cvObject pos tag} {
        #
    variable debugMode    
        #
    ...
    $cvObject  create   circle  $myPosition     [list -radius 5  -tags $myTag  -outline darkred  -width 0.1]
        ...
        #appUtil::pdict $tubeDictionary
        #
    set dict_centerLine     [dict get $tubeDictionary segment]
        #
    set line_pointList      {}
    set center_pointList    {}
        #
        #appUtil::pdict $dict_centerLine
...
proc control::create_dictShape {cvObject pos tag} {
        #
    variable debugMode    
        #
    set tubeDictionary  [model::getDictionary]
    ...
    $cvObject  create   circle      $myPosition [list -radius 5  -tags $myTag  -outline darkred  -width 0.1]
        ...
        # appUtil::pdict $tubeDictionary
        #
    set shapeLeft           [join [dict get $tubeDictionary shape shapeLeft]  " "]
    set shapeRight          [join [dict get $tubeDictionary shape shapeRight] " "]
        #
    set line_pointList      {}
    set center_pointList    {}
        #
...
proc control::create_refPath {cvObject pos direction path tag} {
        #
    set myPosition  $pos
...
proc control::create_refProfile {cvObject pos tag} {
        #
    set myPosition  $pos
    set myTag       $tag
        #
    set profileDictionary   $model::profileDictionary
        #
        # appUtil::pdict $profileDictionary
        #
    set indexList   [lsort -real [dict keys $profileDictionary]]
    ...
    set pointList   {}
        #
    set startDict           [dict get $profileDictionary [lindex $indexList 0]]
    ...
    foreach key $indexList {
            #
        set keyDict         [dict get $profileDictionary $key]
            #
            # puts " --- $key ---"
            # appUtil::pdict $keyDict
...
proc control::update_board {} {
    ...
    create_refProfile   $cvObject $myPosition $myTag
        #
    set myPosition      {30 200}
    create_dictPath     $cvObject $myPosition $myTag
    create_dictShape    $cvObject $myPosition $myTag
