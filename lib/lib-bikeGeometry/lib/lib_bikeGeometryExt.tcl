 ...
 #
    ...
    proc bikeGeometry::_create_GeometryCenter {} {
            ...
            set SeatPost(Height)            [ expr $Geometry(Saddle_Height) - $Saddle(Height) ]
                # set Position(Saddle_Mount)      [ list [expr -1.0 * $Geometry(Saddle_Distance)] $SeatPost(Height) ]
                # set Position(SeatPost_Pivot)    [ vectormath::addVector $Position(Saddle_Mount)  [list 0 $SeatPost(PivotOffset)] -1]
    ...
    proc bikeGeometry::fill_resultValues {} {
            ...
            set Geometry(SeatTube_TubeHeight)   $value
                #
                # --- SeatTube/TopTube - Classic --------------------------
                #
            set position_BB          {0 0}
            set Position(SeatTube_ClassicTopTube)   [ vectormath::intersectPoint $position_BB $Position(SeatPost_SeatTube) $Position(TopTube_End) [vectormath::addVector $Position(TopTube_End) {1 0}] ]
            set value       [ format "%.3f" [vectormath::length $Position(TopTube_End) $Position(SeatTube_ClassicTopTube)]]
            set Geometry(TopTube_LengthClassic)  $value
            set value       [ format "%.3f" [vectormath::length $position_BB $Position(SeatTube_ClassicTopTube)]]
            set Geometry(SeatTube_LengthClassic) $value
                #
                # --- SeatTube/TopTube - Virtual --------------------------
                #
            set Position(SeatTube_VirtualTopTube)   [ vectormath::intersectPoint $position_BB $Position(SeatPost_SeatTube) $Position(HeadTube_End) [vectormath::addVector $Position(HeadTube_End) {1 0}] ]
            set value       [ format "%.3f" [vectormath::length $Position(HeadTube_End) $Position(SeatTube_VirtualTopTube)]]
            set Geometry(TopTube_LengthVirtual)  $value
            set value       [ format "%.3f" [vectormath::length $position_BB $Position(SeatTube_VirtualTopTube)]]
            ...
            #set Geometry(TopTube_LengthVirtual)   [ format "%.3f" $value ]
                #
                # --- HeadTube/OffsetTopTube ---------------
                #
            set value       [vectormath::length $Position(HeadTube_End) $Position(TopTube_End) ]
            set Geometry(HeadTube_CenterTopTube)   [ format "%.3f" $value ]
                #
                # --- HeadTube/OffsetDownTube ---------------
                #
            set value       [vectormath::length $Position(HeadTube_Start) $Position(DownTube_End) ]
            set Geometry(HeadTube_CenterDownTube)   [ format "%.3f" $value ]
                #
                # --- Saddle ------------------------------------------
                #
            ...
            set Geometry(HeadTube_Length)   $HeadTube(Length)
            set Geometry(HeadSet_Bottom)    $HeadSet(Height_Bottom)
                #
                # --- FrontWheel/Radius -------------------------------
                #
            set BB_Position                 {0 0}
    ...
    proc bikeGeometry::__remove__create_DerailleurMountFront {} {
                #
            variable Geometry
            variable Position
                #
    ...
    proc bikeGeometry::__remove__create_CarrierMountFront {} {
                #
            variable Position
                #
            variable Fork
    ...
    proc bikeGeometry::__remove__create_CarrierMountRear {} {
                #
            variable Position
                #
            variable RearWheel
    ...
    proc bikeGeometry::__remove__create_BrakePositionRear {} {
                #
            variable Direction
            variable Geometry
            variable Polygon
            ...
            set pt_02           $Position(SeatStay_End)
                #
            set h_vct   [vectormath::parallel $Position(SeatStay_Start) $Position(SeatStay_End) [expr 0.5 * $SeatStay(DiameterST)] left]
            set pt_04 [lindex $h_vct 0]
            set pt_03 [lindex $h_vct 1]
            ...
            set Position(RearBrake_Mount)        $pt_14
                #
                # puts "    \$Position(RearBrake_Shoe)   $Position(RearBrake_Shoe)"
                # puts "    \$Position(RearBrake_Mount)  $Position(RearBrake_Mount)"
                #
                #
    ...
    proc bikeGeometry::__remove__create_BrakePositionFront {} {
                #
            variable Direction
            variable Geometry
            variable Position
