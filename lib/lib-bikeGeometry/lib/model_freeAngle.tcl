 ...
 namespace eval bikeGeometry::model_freeAngle {
    ...
    proc bikeGeometry::model_freeAngle::create_GeometryFront {} {
            ...
            set Geometry(FrontWheel_y)      [expr {$Geometry(BottomBracket_Depth) - $Geometry(RearWheel_Radius) + $Geometry(FrontWheel_Radius)}]                
                #
            set Position(HandleBar)         [list $Geometry(HandleBar_Distance) [expr {$Geometry(HandleBar_Height) - $Geometry(BottomBracket_Offset_Excenter)}]]    
                #
            ...
            set vect_03 [expr {$vect_01 / sin($Geometry(HeadTube_Angle) * $vectormath::CONST_PI / 180)}]
                #
            set Steerer(Handlebar)          [list  [expr {$Geometry(HandleBar_Distance) - $vect_03}] [expr {$Geometry(HandleBar_Height) - $Geometry(BottomBracket_Offset_Excenter)}] ]
                #
    ...
    proc bikeGeometry::model_freeAngle::create_GeometryCenter {} {
            ...
            if {$Saddle(Height) < 0} {
                ...
                #
            set Position(Saddle)            [list [expr {-1.0 * $Geometry(Saddle_Distance)}]  [expr {$Geometry(Saddle_Height) - $Geometry(BottomBracket_Offset_Excenter)}]]
            set Position(SaddleNose)        [vectormath::addVector  $Position(Saddle) [list [expr {$Saddle(NoseLength) + $Saddle(Offset_x)}] -15]]
                #
            set SeatPost(Height)            [expr {$Geometry(Saddle_Height) - $Geometry(BottomBracket_Offset_Excenter) - $Saddle(Height)}]
            ...
            set Direction(SeatTube)         [vectormath::unifyVector $Position(SeatTube_Start) $Position(SeatPost_SeatTube) ]
                #
            set Position(CrankSet)          [vectormath::addVector $Position(BottomBracket)  [list 0 $Geometry(BottomBracket_Offset_Excenter)] -1]
                #
            ...
            set LegClearance(Position)      [list $TopTube(PivotPosition)  [expr {$Geometry(Inseam_Length) - ($Geometry(RearWheel_Radius) - $Geometry(BottomBracket_Depth))}]]
                #
                #
            set Position(SaddleProposal)    [vectormath::rotateLine $Position(CrankSet)  [expr {0.88*$Geometry(Inseam_Length)}]  [expr {180 - $Geometry(SeatTube_Angle)}]]
                #
            set help_08  [vectormath::addVector    $Position(BottomBracket_Ground) {200 0}]
                #
    ...
    proc bikeGeometry::model_freeAngle::create_GeometryResult {} {
            ...
            set Geometry(SeatTube_LengthVirtual)  [format "%.3f" $value]
                #
                # --- Saddle ------------------------------------------
                #
            ...
            set value       [expr {[lindex $position_HandleBar 0] - 1.0 * [lindex $position_SaddleNose 0]}]
