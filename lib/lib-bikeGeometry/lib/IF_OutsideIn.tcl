        ...
        switch -exact $object {
            Geometry {
                    switch -exact $key {
                        ...
                        {SeatTube_LengthVirtual}        {   bikeGeometry::set_Classic_SeatTubeVirtualLength  $newValue; return [get_Scalar $object $key] }
                        ...
                        default {
                            puts "\n   -<W>- ::bikeGeometry::IF_OutsideIn: Geometry $key ... not registered\n"
                        }
                    }
                }
            RearWheel {
                    switch -exact $key {
                        ...
                        default {
                            puts "\n   -<W>- ::bikeGeometry::IF_OutsideIn: RearWheel $key ... not registered\n"
                        }
                    }
                }
            Reference {
                    switch -exact $key {
                        ...
                        default {
                            puts "\n   -<W>- ::bikeGeometry::IF_OutsideIn: Reference $key ... not registered\n"
                        }
                    }
                }
            default {
                puts "\n   -<W>- ::bikeGeometry::IF_OutsideIn: $object $key ... not registered\n"
            }
        }
            #
    ...
    proc bikeGeometry::set_Default_HeadTubeCenterDownTube   {value} {
                #
            variable Config
            variable Geometry
            variable HeadTube
            ...
            variable Position
                #
            puts "    <1> set_Default_DownTubeOffsetHeadTube   ... check $Geometry(HeadTube_CenterDownTube) ->  $value"
                #
            if {1} {
                    #
                    #
                puts "\n<D>\n<D>\n<D>         ... implementation not exact now   \n<D>\n<D>\n<D>\n"    
                    #
                    #
                    set pt_01       [ vectormath::addVector         $Position(HeadTube_Start) $Direction(HeadTube) $value]
                    set pt_02       [ vectormath::cathetusPoint     {0 0}  $pt_01 [expr {1.0 * $DownTube(OffsetBB)]}]    ;# DownTube Vector
                    #
                    set vct_dt      [ vectormath::parallel $pt_02 $pt_01 [expr {0.5 * $DownTube(DiameterHT)]}]
                    #
                    ...
                    # set vct_ht      [ vectormath::parallel $Position(HeadTube_Start) $Position(HeadTube_End) [expr 0.5 * $HeadTube(Diameter)] left]
                    #
                    puts "      -> \$vct_dt  $vct_dt   [vectormath::dirAngle [lindex $vct_dt 0] [lindex $vct_dt 1]]"
                    #
                    set pt_is       [ vectormath::intersectPoint    [lindex $vct_dt 0] [lindex $vct_dt 1] [lindex $vct_ht 0] [lindex $vct_ht 1]]
                    #
                    puts "      -> \$pt_is   $pt_is   "
                    #
                    set l_offset    [ vectormath::length $pt_is $Position(_Edge_HeadTubeBack_Bottom)]
                    # set vct_cl      [ list $pt_02 $pt_01]                        ;# DownTube centerline Vector
                        #
                ...
                # set delta                   [expr $value - $Geometry(HeadTube_CenterDownTube)]
                    #
                ...
                # set offsetHeadTube_y        [expr $delta   * sin([vectormath::rad $Geometry(HeadTube_Angle)])]
                    ...
                    # puts "         ... \$Geometry(HeadTube_Angle) $Geometry(HeadTube_Angle)"
                    #
                set DownTube(OffsetHT)              $l_offset
                    #
                bikeGeometry::update_Geometry
                    #
                # set Geometry(HeadTube_CenterDownTube)    $value
            }
                #
            puts "    <2> set_Default_DownTubeOffsetHeadTube   ... check $Geometry(HeadTube_CenterDownTube) ->  $value"
            #puts "    <2> set_Default_DownTubeOffsetHeadTube   ... check ->  $value"
                #
            return $Geometry(HeadTube_CenterDownTube)
                #    
    }
