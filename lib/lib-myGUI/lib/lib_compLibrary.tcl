 ...
 #
 # ---------------------------------------------------------------------------
 #  namespace:  myGUI::compLibrary
 # ---------------------------------------------------------------------------
 #
 #

 namespace eval myGUI::compLibrary {

    variable    my_cvObject     {}
    variable    menueFrame      {}
...
#-------------------------------------------------------------------------
   ...
   #
proc myGUI::compLibrary::createLibrary {w} {
    variable my_cvObject
    variable menueFrame
    ...
    ttk::labelframe $menueFrame.bf.update   -text "Update"
        ...
        pack $menueFrame.bf.scale \
             $menueFrame.bf.update -expand yes  -side top  -fill x  -pady 4
    ...
    pack    $menueFrame.bf.scale.reset \
            -side top
        #
    set my_cvObject [cad4tcl::new  $canvasFrame  550  550  A3  1.0  40  -bd 2  -bg lightgray  -relief sunken]
        #
    set tabID       [file tail [string map {. /} $w]]
    set notebook    [winfo parent $w]
    myGUI::gui::register_external_cadCanvas $notebook $tabID  $my_cvObject
        #
    ...
    bind $tree_User         <<TreeviewOpen>>        [list [namespace current]::::populateTree %W]
    bind $tree_User         <<TreeviewSelect>>      [list [namespace current]::::selectTree   %W]
...
#-------------------------------------------------------------------------
    #  populateRoot
proc myGUI::compLibrary::populateRoot {treeWidget title baseDir} {
      # puts "  -> $baseDir"
...
proc myGUI::compLibrary::populateTree {tree {node {}}} {
        #
    variable list_Selection
        # puts "  \n -> populateTree: $node "
        #
...
#-------------------------------------------------------------------------
    #  selectTree
    #
proc myGUI::compLibrary::selectTree {treeWidget} {
...
#-------------------------------------------------------------------------
    #  fillList
    #
proc myGUI::compLibrary::fillList {listWidget path} {
        #
...
#-------------------------------------------------------------------------
    #  selectList
    #
proc myGUI::compLibrary::selectList {listWidget} {
        #
    variable compFile    
        #
...
#-------------------------------------------------------------------------
   ...
   #
proc myGUI::compLibrary::update_compList {{mode {}}} {
        #
    variable    tree_System
    variable    tree_Custom
    variable    tree_User
    ...
    populateRoot $tree_System   System $::APPL_Config(COMPONENT_Dir)
    populateRoot $tree_User     User   [file join $::APPL_Config(USER_Dir)      components] 
    populateRoot $tree_Custom   Custom [file join $::APPL_Config(CUSTOM_Dir)    components] 
        #  
}
...
#-------------------------------------------------------------------------
    #  reset Positioning
    #    
proc myGUI::compLibrary::cleanupTree {treeWidget} {
        # puts "  ... $treeWidget "
...
#-------------------------------------------------------------------------
    #  reset Positioning
    #
proc myGUI::compLibrary::reset_Positioning {} {
        #
    variable rdials_list
    variable configValue
...
#-------------------------------------------------------------------------
   ...
   #
proc myGUI::compLibrary::refitCanvas {} {
        #
    variable my_cvObject
        #
    puts "   ... myGUI::compLibrary::refitCanvas: $my_cvObject"
    $my_cvObject    fit
        #
}
...
#-------------------------------------------------------------------------
    #  update Canvas
    #
proc myGUI::compLibrary::updateCanvas {{entryVar ""} {value {0}} {drag_Event {}}} {
        #
    variable my_cvObject
    variable compFile
    variable configValue
        #
    set cvCanvas    [$my_cvObject getCanvas] 
        #
    if {$entryVar ne ""} {
        set $entryVar $value
    }
    ...
    if {$compFile != {}} {
        ...
        set compPosition    [list $configValue(compOffset_X) [expr 1.0*$configValue(compOffset_y)]]
            #
        set myComponent     [$my_cvObject create svg $compPosition [list -svgFile $compFile  -angle $configValue(compAngle)  -tags __Decoration__]]
            #
        moveto_StageCenter  $myComponent
            #
        foreach cv_Item [$my_cvObject find withtag __Decoration__] {
                # puts "  -> $cv_Item"
            set cv_Type     [$cvCanvas type $cv_Item]
            switch -exact $cv_Type {
                oval     -
                polygon  { $my_cvObject itemconfigure  $cv_Item -fill $myGUI::view::edit::colorSet(components) }
                default  {}
            }
...
#-------------------------------------------------------------------------
    ...
    #
proc myGUI::compLibrary::moveto_StageCenter {item} {
        #
    variable my_cvObject
        #
    set w           [$my_cvObject getCanvas]
        #
    set stageCenter [$my_cvObject getCenter]
    set bottomLeft  [$my_cvObject getBottomLeft]
        #
...
#-------------------------------------------------------------------------
    #  create Centerline
    #
proc myGUI::compLibrary::create_Centerline {} {
        #
    variable my_cvObject
        #
    set stage           [$my_cvObject getCanvas]
    set stageCenter     [$my_cvObject getCenter]
    set stageFormat     [$my_cvObject configure Stage Format]
    set stageWidth      [$my_cvObject configure Stage Width]
    set stageHeight     [$my_cvObject configure Stage Height]
    set stageScale      [$my_cvObject configure Canvas Scale]
        #
    set bottomLeft      [$my_cvObject getBottomLeft]
        #
    set formatWidth     $stageWidth
    set formatHeight    $stageHeight
        #
    set cl_x0    10
    ...
    set cl_y1    [expr $formatHeight -10]
        #
    $my_cvObject    create centerline [list $cl_x0 $cl_yc $cl_x1 $cl_yc ]  -fill red  -tags __CenterLine__
    $my_cvObject    create centerline [list $cl_xc $cl_y0 $cl_xc $cl_y1 ]  -fill red  -tags __CenterLine__
        #
}
...
#-------------------------------------------------------------------------
    #  create config_line
    #
proc myGUI::compLibrary::create_config_line {w lb_text entryVar start current end resolution command} {
        #
    variable    rdials_list
