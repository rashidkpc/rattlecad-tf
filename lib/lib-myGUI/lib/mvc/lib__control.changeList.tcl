...
# 
    ...
    namespace eval myGUI::control::changeList {
     ...
     #-------------------------------------------------------------------------
        #  append undoList
    proc myGUI::control::changeList::append {parameter oldValue newValue} {
        variable _editList
    ...
    #-------------------------------------------------------------------------
        #  previous 
    proc myGUI::control::changeList::previous {} {
        variable _editList
        variable _listIndex
            #
		...
		if  {$_listIndex > 0} {
            ...
            puts "           ... use entry:  [format " (%3s) ...  %40s  %-25s"  $_listIndex $parameter $oldValue]"
              # set oldValue
            myGUI::control::setValue [list $parameter $oldValue] {update} {skip}
              #
            incr _listIndex -1
              #
        } else {
    ...
    #-------------------------------------------------------------------------
        #  next 
    proc myGUI::control::changeList::next {} {
		variable _editList
		variable _listIndex
            #
		...
		if  {$_listIndex <= [array size _editList] } {
			...
			puts "           ... use entry:  [format " (%3s) ...  %40s  %-25s"  $_listIndex $parameter $newValue]"
			  # set newValue
			myGUI::control::setValue [list $parameter $newValue] {update} {skip}
			  #
		} else {
			puts "           <I> EXCEPTION: ... $_listIndex"
		}
    ...
    #-------------------------------------------------------------------------
        #  reset undoList
    proc myGUI::control::changeList::reset {} {
		variable _editList
		variable _listIndex         ;# current index in _editList
            #
    ...
    #-------------------------------------------------------------------------
        #  print undoList 
    proc myGUI::control::changeList::print {} {
		variable _editList
		variable _listIndex         ;# current index in _editList
    ...
    #-------------------------------------------------------------------------
        #  get_undoStack
    proc myGUI::control::changeList::get_undoStack {} {        
		variable _editList
		variable _listIndex
		  # print
    ...
    #-------------------------------------------------------------------------
        #  get_undoStack
    proc myGUI::control::changeList::get_changeIndex {} {        
		variable _listIndex
		return  $_listIndex
    }
