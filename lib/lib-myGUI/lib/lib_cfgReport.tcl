 ...
 #
 # ---------------------------------------------------------------------------
 #  namespace:  myGUI::cfg_report
 # ---------------------------------------------------------------------------
 #
 #


 namespace eval myGUI::cfg_report {
    ...
    #-------------------------------------------------------------------------
       ...
       #
    proc myGUI::cfg_report::createReport {w} {
        variable treeWidget
        variable menueFrame
        ...
        ttk::button  $menueFrame.open    -text {Open xml-File}                   -width 30   -command { myPersist::file::get_XMLContent {}    visualize}
        ttk::button  $menueFrame.bt01    -text {cadCanvas}                       -width 30   -command { myGUI::cfg_report::fillTree_Variable  cadCanvas}
        ttk::button  $menueFrame.bt02    -text {rattleCAD_init.xml}              -width 30   -command { myGUI::cfg_report::fillTree_Variable $APPL_Config(root_InitDOM) }
        ttk::button  $menueFrame.bt03    -text {Template Road}                   -width 30   -command { myPersist::file::get_XMLContent [myPersist::file::getTemplateFile Road]     visualize}
        ttk::button  $menueFrame.bt04    -text {Template OffRoad}                -width 30   -command { myPersist::file::get_XMLContent [myPersist::file::getTemplateFile MTB ]     visualize}


        ttk::button  $menueFrame.bt06    -text {current Project}                 -width 30   -command { myGUI::cfg_report::fillTree_Variable {currentProject} }
        ttk::button  $menueFrame.bt07    -text {rattleCAD - Runtime}             -width 30   -command { myGUI::cfg_report::fillTree_Variable {runTime} }
        ttk::button  $menueFrame.bt08    -text {OS - Environment}                -width 30   -command { myGUI::cfg_report::fillTree_Variable {osEnv} }
        ttk::button  $menueFrame.clear   -text {clear Tree}                      -width 30   -command { myGUI::cfg_report::cleanupTree }
        
              # $menueFrame.open \
                $menueFrame.bt03 \
                $menueFrame.bt04 \
                
        pack    $menueFrame.bt01 \
                $menueFrame.bt02 \
                $menueFrame.bt06 \
                $menueFrame.bt07 \
                $menueFrame.bt08 \
    ...
    #-------------------------------------------------------------------------
       #  cleanup Tree
       #
    proc myGUI::cfg_report::cleanupTree {} {
        variable treeWidget
            # puts "  ... $treeWidget "
    ...
    #-------------------------------------------------------------------------
       #  fill Tree - Variable
       #
    proc myGUI::cfg_report::fillTree_Variable {var} {
            #
            # -- define global parameters
            #
        switch -exact $var {
            currentProject {
					set var [myGUI::model::model_Info::getDOMNode      persistenceRoot]
                    # set var $myGUI::modelAdapter::persistRoot
                    # set var [[myGUI::model::model_Info::getProjectDoc] documentElement]
                    # set var $myGUI::modelAdapter::persistenceDOM
                    # set var $myGUI::control::currentDOM  ;# 3.4.03 
                }
            cadCanvas {
                    set var [[cad4tcl::getReportDOC] documentElement]
                }
            runTime {
            ...
            #
        myGUI::cfg_report::fillTree "$var" root
            #
    }
    ...
    #-------------------------------------------------------------------------
       #  fill Tree - File
       #
    proc myGUI::cfg_report::fillTree {node parent} {
        variable treeWidget
        cleanupTree
    ...
    #-------------------------------------------------------------------------
       ...
       #
    proc myGUI::cfg_report::recurseInsert {w node parent} {
