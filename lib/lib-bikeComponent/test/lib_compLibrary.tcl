 ...
 #
 # ---------------------------------------------------------------------------
 #  namespace:  myTest::compLibrary
 # ---------------------------------------------------------------------------
 #
 #

namespace eval myTest::compLibrary {
    ...
    set         dir_System      [file join [set [namespace parent]::packageHomeDir] components]
 ...
 #-------------------------------------------------------------------------
   ...
   #
proc myTest::compLibrary::createLibrary {w} {
    variable cvObject
    variable compCanvas
    variable menueFrame
    ...
    # set retValue  [ canvasCAD::newCanvas    cv_Component $canvasFrame  "Components" 550 550 A3 1  40  -bd 2   -bg lightgray  -relief sunken ]
        #
    # foreach {w compCanvas} $retValue break
        #
    ...
    set tabID       [string map {/ . } [string map {. /} $w] ]
        ...
        # exit
    # myTest::gui::register_external_canvasCAD $tabID $compCanvas
 ...
 #-------------------------------------------------------------------------
    #  set component color
proc myTest::compLibrary::setComponentColor {color} {
...
proc myTest::compLibrary::setUserDir {dir} {
    variable    dir_User
    set         dir_User  $dir
}        
proc myTest::compLibrary::setCustomDir {dir} {
    variable    dir_Custom
    set         dir_Custom  $dir
}
 ...
 #-------------------------------------------------------------------------
    #  populateRoot
proc myTest::compLibrary::populateRoot {treeWidget title baseDir} {
      # puts "  -> $baseDir"
...
proc myTest::compLibrary::populateTree {tree {node {}}} {
    variable list_Selection
 ...
 #-------------------------------------------------------------------------
    #  selectTree
    #
proc myTest::compLibrary::selectTree {treeWidget} {
...
#-------------------------------------------------------------------------
    #  fillList
    #
proc myTest::compLibrary::fillList {listWidget path} {
 ...
 #-------------------------------------------------------------------------
    #  selectList
    #
proc myTest::compLibrary::selectList {listWidget} {
 ...
 #-------------------------------------------------------------------------
   ...
   #
proc myTest::compLibrary::update_compList {{mode {}}} {
        #
    variable    tree_System
    variable    tree_Custom
    variable    tree_User
    ...
    cleanupTree  $list_Selection
        #
    populateRoot $tree_System   System $dir_System  ;# ::APPL_Config(COMPONENT_Dir)
    populateRoot $tree_User     User   $dir_User    ;#[file join $::APPL_Config(USER_Dir)      components]
    populateRoot $tree_Custom   Custom $dir_Custom  ;#[file join $::APPL_Config(CUSTOM_Dir)    components]
        #    
}
...
#-------------------------------------------------------------------------
    #  reset Positioning
    #    
proc myTest::compLibrary::cleanupTree {treeWidget} {
        # puts "  ... $treeWidget "
 ...
 #-------------------------------------------------------------------------
   #  reset Positioning
   #
proc myTest::compLibrary::reset_Positioning {} {
    variable rdials_list
    variable configValue
 ...
 #-------------------------------------------------------------------------
   ...
   #
proc myTest::compLibrary::refitCanvas {} {
    variable compCanvas
 ...
 #-------------------------------------------------------------------------
   #  update Canvas
   #
proc myTest::compLibrary::updateCanvas {{entryVar ""} {value {0}} {drag_Event {}}} {
    variable cvObject
    ...
    if {$compFile != {}} {
        ...
        set myComponent     [$cvObject create svg $compPosition [list -svgFile $compFile  -angle $configValue(compAngle)  -tags __Decoration__]]
        #
        ...
        foreach cv_Item [$cvObject find withtag __Decoration__] {
            ...
            switch -exact $cv_Type {
                oval     -
                polygon  { $cvObject itemconfigure  $cv_Item -fill $compColor }
                default  {}
            }
 ...
 #-------------------------------------------------------------------------
   ...
   #
proc myTest::compLibrary::moveto_StageCenter {item} {
        #
    variable cvObject
        #
 ...
 #-------------------------------------------------------------------------
   #  create Centerline
   #
proc myTest::compLibrary::create_Centerline {} {
    variable  cvObject
    ...
    set stageCenter     [$cvObject getCenter]
    set stageFormat     [$cvObject getStageSize]
    ...
    $cvObject create centerline [list $cl_x0 $cl_yc $cl_x1 $cl_yc ]  -fill red  -tags __CenterLine__
    $cvObject create centerline [list $cl_xc $cl_y0 $cl_xc $cl_y1 ]  -fill red  -tags __CenterLine__
}

 #-------------------------------------------------------------------------
   #  create config_line
   #
proc myTest::compLibrary::create_config_line {w lb_text entryVar start current end resolution command} {
    variable    rdials_list
