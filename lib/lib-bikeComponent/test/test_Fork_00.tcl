...
# 
    ...
    proc myTest::main {} {
        ...
        # --------------------------------------------
            ...
            set retValue [canvasCAD::newCanvas cv01  $f2_canvas.cv01 	"MyCanvas_02"  1080  800 	A2 1.0 40 -bd 2  -bg white  -relief sunken ]
            ...
            ttk::button  $f2_config.stem        -text {Stem}                -width 20   -command [list myTest::showComponent Stem]
                #
            ttk::button  $f2_config.forkBent        -text {ForkCustom_bent}     -width 20   -command [list myTest::showForkOld   SteelCustomBent]
            ttk::button  $f2_config.forkStraight    -text {ForkCustom_straight} -width 20   -command [list myTest::showForkOld   SteelCustomStraight]
            ttk::button  $f2_config.forkMAX         -text {ForkMAX}             -width 20   -command [list myTest::showForkOld   SteelLuggedMAX]
            ttk::button  $f2_config.forkComp        -text {ForkComposite}       -width 20   -command [list myTest::showForkOld   Composite]
            ttk::button  $f2_config.forkTusk        -text {ForkComposite_Tusk}  -width 20   -command [list myTest::showForkOld   Composite_TUSK]
            ttk::button  $f2_config.forkSusp26      -text {ForkSuspension_26}   -width 20   -command [list myTest::showForkOld   Suspension_26]
            ttk::button  $f2_config.forkSusp20      -text {ForkSuspension_20}   -width 20   -command [list myTest::showForkOld   Suspension_20]
            ...
            pack    $f2_config.refit \
                    ...
                    $f2_config.space_03 \
                    $f2_config.forkBent \
                    $f2_config.forkStraight \
                    $f2_config.forkMAX    \
                    $f2_config.forkComp   \
                    $f2_config.forkTusk   \
                    $f2_config.forkSusp26 \
                    $f2_config.forkSusp20 \
    ...
    proc myTest::showComponent {compType} {
        switch -exact $compType {
                Saddle              {   set compSVG     [bikeComponent::get_Component Saddle      XZ etc:saddle/selle_san_marco_concor.svg       ]}
    ...
    proc myTest::showMultiComponent {compType} {
	    ...
	    #
    proc myTest::showForkOld {forkType} {
            #
        puts "   -> myTest::showForkOld - $forkType"    
            #
            #
        switch $forkType {
            SteelCustomBent {
                    set forkType SteelCustom
                    set bikeComponent::Fork::Config(ForkBlade) bent
                }
            SteelCustomStraight {
                    set forkType SteelCustom
                    set bikeComponent::Fork::Config(ForkBlade) straight 
                }
            default {}
        }
            #
        bikeComponent::Fork::update $forkType
            #
        set polygon_Blade {}    
            #
            #
        switch $forkType {
            ...
            Composite_TUSK  -
            Suspension_*     {
                    set comp_Crown     [bikeComponent::Fork::get_Component  ForkCrown  ] 
                    set comp_DropOut   [bikeComponent::Fork::get_Component  ForkDropout] 
                    set polygon_Blade  [bikeComponent::Fork::get_Blade] 
                }
            default {
                    set comp_Crown     [bikeComponent::Fork::get_Component  ForkCrown  ]
                    set comp_DropOut   [bikeComponent::Fork::get_Component  ForkDropout]
        ...
        $myTest::cv02 clean_StageContent
            #
        puts "  -> \$comp_Crown ....... $comp_Crown"    
        puts "  -> \$comp_DropOut ..... $comp_DropOut"    
        puts "  -> \$polygon_Blade .... $polygon_Blade"    
            #
        set path_Crown      [bikeComponent::get_ComponentPath   ForkCrown   $comp_Crown]
        set path_DropOut    [bikeComponent::get_ComponentPath   ForkDropout $comp_DropOut]
            #
        puts "  -> \$path_Crown ....... $path_Crown"    
        puts "  -> \$path_DropOut ..... $path_DropOut"
            #
        set myCrown     [$myTest::cv02 readSVG $path_Crown    {297  400}] 
        set myDropout   [$myTest::cv02 readSVG $path_DropOut  {342   35}]
            #
        set cv_Scale    [$myTest::cv02 getNodeAttr Canvas    scale]
        puts "   -> $cv_Scale"
        set myBlade     [$myTest::cv02 create polygon $polygon_Blade -fill white  -outline black  -tags __Decoration__]
        #$myTest::cv02 move $myCrown     297 -200
        #$myTest::cv02 move $myDropout   342 -200
        $myTest::cv02 move $myBlade     [expr 297.0/$cv_Scale]  [expr -250.0/$cv_Scale]
        #$myTest::cv02 readSVG $comp_DropOut    
            #
        return    
            #
    }
