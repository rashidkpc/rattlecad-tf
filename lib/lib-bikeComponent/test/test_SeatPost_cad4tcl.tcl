...
# 
    ...
    #
    #
set WINDOW_Title      "test bikeComponent - Seatpost, based on cad4tcl@rattleCAD"
    #
    #
...
namespace eval control {
    ...
    set myObject        [bikeComponent::SeatPost    new]
    
    variable length         [$myObject getValue Scalar Length     ]       
    variable diameter       [$myObject getValue Scalar Diameter   ]             
    variable pivotOffset    [$myObject getValue Scalar PivotOffset]
    variable setback        [$myObject getValue Scalar Setback    ]
...
proc control::update_board {{value {0}}} {
    ...
    set pos_ShaftStart [$myObject getValue Position      ShaftStart]
    set pos_ShaftEnd   [$myObject getValue Position      ShaftEnd]
        #
    puts "    -> \$pos_Origin       $pos_Origin"    
    puts "    -> \$pos_ShaftStart   $pos_ShaftStart"    
    puts "    -> \$pos_ShaftEnd     $pos_ShaftEnd"    
        #
    set pos_Origin     [vectormath::addVector $pos_Offset   $pos_Origin]      
    set pos_ShaftStart [vectormath::addVector $pos_Offset   $pos_ShaftStart]     
    set pos_ShaftEnd   [vectormath::addVector $pos_Offset   $pos_ShaftEnd]     
    set pos_Mount      [vectormath::addVector $pos_Origin   [list 0 $pivotOffset]]
    ...
    set myItem_04   [$cvObject create circle $pos_ShaftStart -radius 2  -outline blue  -tags __CenterLine__]
    set myItem_05   [$cvObject create circle $pos_ShaftEnd   -radius 2  -outline blue  -tags __CenterLine__]
    set myItem_06   [$cvObject create circle $pos_Mount      -radius 1  -outline orange  -tags __CenterLine__]
        #
    set myDim_01    [$cvObject create dimensionLength \
                                            [appUtil::flatten_nestedList   $pos_ShaftStart $pos_Origin] \
                                            [list \
                                                horizontal \
                                                [expr   60 * $drw_scale]   [expr   0 * $drw_scale]  \
                                                gray50]]                                                                                                              
    set myDim_02    [$cvObject create dimensionLength \
                                            [appUtil::flatten_nestedList   $pos_ShaftEnd $pos_Origin] \
                                            [list \
                                                vertical \
                                                [expr   -45 * $drw_scale]   [expr   -5 * $drw_scale]  \
                                                gray50]]                                                                                                              
    set myDim_03    [$cvObject create dimensionLength \
                                            [appUtil::flatten_nestedList   $pos_Mount $pos_Origin] \
                                            [list \
                                                vertical \
    ...
    [namespace current]::moveto_StageCenter [list \
                                                $myComponent \
                                                $myItem_03 $myItem_04 $myItem_05 $myItem_06 \
...
proc control::setComponentScalar {entryVar args} {
    ...
    switch -exact $entryKey {
        length      {   $myObject setValue Scalar Length        $entryValue }       
        diameter    {   $myObject setValue Scalar Diameter      $entryValue }             
        pivotOffset {   $myObject setValue Scalar PivotOffset   $entryValue }             
        setback     {   $myObject setValue Scalar Setback       $entryValue }             
        default         {}
    }
        #
    $myObject update
...
proc view::create {windowTitle} {
        ...
        view::create_config_line $f_settings.compParameter.l_01   "   length:       "  control::length          100   340   ;#   0
        view::create_config_line $f_settings.compParameter.l_02   "   diameter:     "  control::diameter         10    45   ;#   0
        view::create_config_line $f_settings.compParameter.l_03   "   pivotOffset:  "  control::pivotOffset     -25    50   ;#   0
        view::create_config_line $f_settings.compParameter.l_04   "   setback    :  "  control::setback         -25    40   ;#   0
