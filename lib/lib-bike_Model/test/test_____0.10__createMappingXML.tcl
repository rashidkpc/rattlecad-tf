...
puts "\n\n ====== I N I T ============================ \n\n"
    ...
    set fp [open [file join $packageHomeDir .. etc dataStructure_complete.xml] ]
        ...
        close               $fp
            #
        set mappingDoc      [dom parse $structureXML]
        set mappingRoot     [$mappingDoc documentElement]   
    
        #
    puts [$mappingRoot asXML]
        #
    ...
    puts "\n --- create main mapping --- "
        #
    foreach arrayNode [$mappingRoot childNodes] {
        ...
        foreach typeNode [$arrayNode childNodes] {
            ...
            foreach keyNode [$typeNode childNodes] {
                ...
                puts "         -- geo -> $geometryKey <----"
                    #
                $keyNode setAttribute guiKey            $guiKey
                $keyNode setAttribute geometryXMLKey    $geometryKey
                $keyNode setAttribute geometryModelKey  {}
                $keyNode setAttribute componentKey      {}
                $keyNode setAttribute frameKey          {}
                $keyNode setAttribute geometryKey       {}
                    #
                if 0 {
                    set mapNode     [$mappingDoc createElement gui]
                    set textNode    [$mappingDoc  createTextNode $guiKey]
                    $mapNode    appendChild $textNode
                    $keyNode    appendChild $mapNode
                        #
                    set mapNode     [$mappingDoc createElement geometry]
                    set textNode    [$mappingDoc  createTextNode $geometryKey]
                    $mapNode    appendChild $textNode
                    $keyNode    appendChild $mapNode
                        #
                    set mapNode     [$mappingDoc createElement frame]
                    set textNode    [$mappingDoc  createTextNode {}]
                    $mapNode    appendChild $textNode
                    $keyNode    appendChild $mapNode
                        #
                    set mapNode     [$mappingDoc createElement component]
                    set textNode    [$mappingDoc  createTextNode {}]
                    $mapNode    appendChild $textNode
                    $keyNode    appendChild $mapNode
                }    
