...
puts "\n\n ====== I N I T ============================ \n\n"
    ...
    set modelMapping  [bikeFacade::get_MappingDict gui]   
        #
    set failureList   {}    
        #
    foreach myKey [dict keys $myMapping] {
        set my_targetKey    [dict get $myMapping    $myKey]
        set model_targetKey [dict get $modelMapping $myKey]
        if {$my_targetKey != $model_targetKey} {
            lappend failureList "$my_targetKey - $model_targetKey"
        }
        puts "   $my_targetKey <-> $my_targetKey"
    }
    puts " -- done --"
    puts " $failureList"
        #
        ...
        #
    foreach myKey [dict keys $modelMapping] {
        set my_targetKey    [dict get $myMapping    $myKey]
        set model_targetKey [dict get $modelMapping $myKey]
        if {$my_targetKey != $model_targetKey} {
            lappend failureList "$my_targetKey - $model_targetKey"
    ...
    set myMapping     [dict get [bikeFacade::geometry::getMappingDict] mapping]
    set modelMapping  [bikeFacade::get_MappingDict geometryXML]   
    foreach myKey [dict keys $myMapping] {
        set my_targetKey    [dict get $myMapping    $myKey]
        set model_targetKey [dict get $modelMapping $myKey]
        if {$my_targetKey != $model_targetKey} {
