 ...
 #
 # ---------------------------------------------------------------------------
 #	namespace:  cadCanvas::vectorfont
 # ---------------------------------------------------------------------------
 #
 # 
 #  176 °   Degree_Sign
...
namespace eval vectorfont {
    ...
    variable font_dir       [file normalize [file join $packageHomeDir .. font]]
        # tk_messageBox -message  "  ... $packageHomeDir  $font_dir    "
    variable shape 
    variable glb_shp
    variable glb_dir
    variable fid
    ...
    proc draw_seg { c offs } {
    	...
    	if $glb_state($fid,drmode) {
            #  debug "draw_seg $offs -> $ox $oy $nx $ny"
                # puts "  -> \$useTkpath $useTkpath"
                # puts "  -> [winfo class $c] -> \$linewidth $linewidth"
            switch [winfo class $c] {
                PathCanvas {
                    $c create pline \
                        $ox $oy $nx $ny \
                        -stroke $glb_state($fid,color) \
...
#
    ...
    proc setscale { factor } {
    	...
    	variable fid
            # base size 100
    	set glb_state($fid,sfact) [expr {double($factor/10)}]
    }
    proc setline { width } {
    	variable linewidth
    	set linewidth $width
    ...
    proc setCanvas { canv value} {
    	variable useTkpath
        variable lineType
        if $value {
            switch [winfo class $canv] {
                PathCanvas {
                    set useTkpath 1
                    set lineType pline
                }
