...
namespace eval control {
    ...
    variable typeTube              circle   ;#  ... circle / ellipse
        #
    if 1 {
        variable diameterToolTop          28 
...
proc control::update {{key _any} {value {}}} {
    ...
    if [info exists $key] {
        ...
        #
        #
    $miterObjectOrigin setScalar   Precision            $precision
        #
    set precision   [$miterObjectOrigin getScalar   Precision]
        #
        #
        # -- miterDebugCylinder
        #
    ...
    switch -exact $typeTube {
        ...
        default {
            $miterDebugCylinder setScalar  DiameterTube     $diameterTube_1
            $miterDebugCylinder setScalar  DiameterTube2    $diameterTube_1
        } 
    }
    ...
    switch -exact $typeTube {
        ...
        default {
            $miterDebugPlane    setScalar  DiameterTube     $diameterTube_1
            $miterDebugPlane    setScalar  DiameterTube2    $diameterTube_1
        } 
    }
    ...
    switch -exact $typeTube {
        oval {
            $miterObjectOrigin  setScalar  DiameterTube     $diameterTube_1
            $miterObjectOrigin  setScalar  DiameterTube2    $diameterTube_2
        }
        round -
        default {
            $miterObjectOrigin  setScalar  DiameterTube     $diameterTube_1
            $miterObjectOrigin  setScalar  DiameterTube2    $diameterTube_1
        } 
    }
        #
    if {$typeTool eq {cone}} {
        ...
        if {$k >= 0} {
            ...
            set length_03   [expr  $radius_02 / $k]
                # 
            $miterObjectOrigin  setScalar   HeightToolCone      $length_03 
            ...
            $miterObjectOrigin  setScalar   OffsetToolBase      $length_03 
            $miterObjectOrigin  setScalar   AngleToolPlane      $angleToolPlane
                ...
                # puts "           DiameterTop        [$miterObjectOrigin  getScalar   DiameterTop     ]"    
                ...
                # puts "           OffsetToolBase     [$miterObjectOrigin  getScalar   OffsetToolBase  ]"    
...
proc control::createTube {position} {
    ...
    set profileOrigin   [vectormath::mirrorCoordList    $profileOrigin  x]
    set profileOrigin   [vectormath::rotateCoordList    {0 0}   $profileOrigin  90]
    set profileOrigin   [vectormath::addVectorCoordList $pos(TubeOrigin) $profileOrigin]
        #
    set profileEnd      [$miterObjectOrigin getProfile  End     left]
    set profileEnd      [vectormath::mirrorCoordList    $profileEnd  x]
    set profileEnd      [vectormath::rotateCoordList    {0 0}   $profileEnd     90]
    set profileEnd      [vectormath::addVectorCoordList $pos(TubeEnd)    $profileEnd]
        #
    ...
    set profileOrigin   [vectormath::addVectorCoordList $pos(TubeOrigin)    $profileOrigin]
        #
    set profileEnd      [$miterObjectOrigin getProfile  End     left   opposite]
    set profileEnd      [vectormath::mirrorCoordList    $profileEnd     x]
    set profileEnd      [vectormath::rotateCoordList    {0 0}   $profileEnd     90]
    set profileEnd      [vectormath::addVectorCoordList $pos(TubeEnd)       $profileEnd]
        #
    set rightView_Front  [join "$profileOrigin $profileEnd" " "]    
        #
        #
    set profileOrigin   [$miterObjectOrigin getProfile  Origin  right opposite]
    set profileOrigin   [vectormath::rotateCoordList    {0 0}   $profileOrigin  90]
    set profileOrigin   [vectormath::addVectorCoordList $pos(TubeOrigin) $profileOrigin]
        #
    set profileEnd      [$miterObjectOrigin getProfile  End     right opposite]
    set profileEnd      [vectormath::rotateCoordList    {0 0}   $profileEnd     90]
    set profileEnd      [vectormath::addVectorCoordList $pos(TubeEnd)    $profileEnd]
        #
...
proc control::createDictView {position} {
    ...
    set viewLength          150
        #
    set miterDict   [$miterObjectOrigin getDictionary]
        #
...
proc view::create {windowTitle} {
    ...
    view::create_config_line $f_config.tube.d_1           "Diameter:            "     control::diameterTube_1      20    60   ;#   0
    view::create_config_line $f_config.tube.d_2           "Diameter-2:          "     control::diameterTube_2      30    80   ;#   0
    view::create_config_line $f_config.tube.l             "Length:              "     control::lengthTube         100   250   ;#   0
    
    view::create_config_line $f_config.config.prec        "Precision:           "     control::precision           20   380  ;#  10
    
    radiobutton  $f_config.tube.typeCircle          -text "Circle"          -variable control::typeTube   -command control::update    -value round    
    radiobutton  $f_config.tube.typeEllipse         -text "Ellipse"         -variable control::typeTube   -command control::update    -value oval 

    pack    $f_config.tube.typeCircle       \
            $f_config.tube.typeEllipse      \
        -side top  -fill x
