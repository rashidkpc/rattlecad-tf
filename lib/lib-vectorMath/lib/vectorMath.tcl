...
package provide vectormath 1.01
...
proc vectormath::rotateCoordList { p_cent p_list angle } {
        # {x y x y ...}
    set newList {}
    foreach {x y} $p_list {
...
proc vectormath::addVectorCoordList {v  p_list {scaling 1}} {
        # {x y x y ...}
    set newList {}
    lassign $v x1 y1
...
proc vectormath::scaleCoordList { pRef ptList scale} {
        # {x y x y ...}
        # return vector with length 1 as default
    lassign  $pRef xRef yRef
...
proc vectormath::rotatePointList { p_cent p_list angle } {
    ...
    set PointList {}
    foreach xy $p_list {
        set _xy [rotatePoint $p_cent $xy $angle]
        lappend PointList $_xy
    }
    return $PointList
}
proc vectormath::addVectorPointList {v  p_list {scaling 1}} {
    ...
    lassign $v x1 y1
        # foreach {x1 y1} $v break
        #
    foreach {coord} $p_list { 
        foreach {x y} $coord { 
            set x [expr {$x1 + $scaling*$x}] 
            set y [expr {$y1 + $scaling*$y}]
            lappend newList [list $x $y]
        }
    }
    return $newList
}
proc vectormath::mirrorPointList {p_list axle} {
        # {{x y} {x y} ...}
    set newList {}
    foreach {coord} $p_list { 
        foreach {x y} $coord {
            switch -exact $axle {
                y { set xy [list [expr {-1.0 * $x}] $y]}
                x -
                default {
                    set xy [list $x  [expr {-1.0 * $y}]]
                }
            }
            lappend newList $xy
        }
...
proc vectormath::mirrorCoordList {ptList axle} {
        # {x y x y ...}
    set newList {}
    foreach {x y} $ptList { 
        switch -exact $axle {
...
proc vectormath::rotatePoint {p_cent p_rot angle} { 
        ...
        # angle in degree
    lassign [subVector $p_rot $p_cent]  x y			
        # foreach {x y} [subVector $p_rot $p_cent ]  break			
    set p0  [list $x $y]
        # puts "   -> rotatePoint: VRotate"
        # puts "         -> \$p0     $p0"
        # puts "         -> \$angle  $angle  <-  grad"
...
proc vectormath::dirAngle {p1 p2} { 
    ...
    # angle in degree
    return [dirAngle_degree $p1 $p2]
}    
    
proc vectormath::dirAngle_radiant {p1 p2} { 
    # angle from positive x-axis
    # angle in radiants
    variable CONST_PI
    ...
    if { $x == 0 } {
        if {$y > 0 } {
            return [expr { 0.5 * $CONST_PI}] 
        } else {
            return [expr {-0.5 * $CONST_PI}]
        }
    }
        #       
    set phi  [expr atan(1.0*$y/$x)]
    if { $x <  0 } {
        return [expr {$phi + $CONST_PI}] 
    } else {
        return $phi
    }
}
  
proc vectormath::dirAngle_degree {p1 p2} { 
    # angle from positive x-axis
    # angle in degree
    variable CONST_PI
    set phi [vectormath::dirAngle_radiant $p1 $p2]
    return  [expr {$phi * 180 / $CONST_PI}]
}
  
proc vectormath::dirCarthesian {angle} { 
    # angle from positive x-axis
    # angle in degree
    return [rotatePoint {0 0} {1 0} $angle]
}
...
proc vectormath::perpendicular {p1 p2 distance {direction {right}}} { 
    # return vector perpendicular to dir through p1 with distance 
    #   and direction from p1 to p2
    set pVect   [parallel $p1 $p2 $distance $direction]
    return      [lindex $pVect 0]
}

proc vectormath::perpendicular_local {vct {direction right}} { 
        # return vector in perpendicular direction
        #   and direction from p1 to p2
    lassign $vct dir_x dir_y
        # foreach {dir_x dir_y} $vct break
    switch -exact $direction {
        {right} -
        -1      {set orient -1.0}
        default {set orient  1.0}
    }
    set x       [expr {$orient * $dir_y * -1.0}]
    set y       [expr {$orient * $dir_x}]
        #puts "                     -> $direction -> $orient"
    return      [list $x $y]
}
...
proc vectormath::angle {p1 pc p3} {
    variable CONST_PI
        # puts "  $p1 $pc $p3"
    lassign [subVector $p1 $pc] x1 y1
    lassign [subVector $p3 $pc] x2 y2 
...
proc vectormath::anglePoint {p} {
    set angle [angle {1 0} {0 0} $p]
    return $angle
}
proc vectormath::localVector_2_Degree {locVector} {
        #
    variable  CONST_PI
        #
    lassign $locVector x y
        # foreach {x y} $locVector break
    if {$x == 0} {
        if {$y > 0} {
            return  90
        } else {
...
proc vectormath::bisectAngle {p1 p2 p3} {
    lassign [subVector $p1 $p2] x1 y1
    lassign [subVector $p3 $p2] x2 y2
...
proc vectormath::trisectAngle {p1 p2 p3} {
    ...
    if {$cross < 0} {
        lassign [list $p3 $p1] p1 p3
            # foreach {p1 p3} [list $p3 $p1] break
    }
...
proc vectormath::addVector {v1 v2 {scaling 1}} {
    lassign $v2 x y
    if {$scaling == 1} {
        return [::math::geometry::+ $v1 $v2]
    } else {
        return [::math::geometry::+ $v1 [list [expr {$x * $scaling}] [expr {$y * $scaling}]]]
    }
}	 
           ##+##########################################################################
           #   subVector -- subtract two vectors
proc vectormath::subVector {v1 v2} { 
    return [::math::geometry::- $v1 $v2]
}
           ##+##########################################################################
...
proc vectormath::vector {arc} {
    set xy [rotatePoint {0 0} {1 0} $arc]
    return $xy
}
...
proc vectormath::VRotate {v beta {unit {rad}}} {
    ...
    #
proc vectormath::flatten_nestedList { args } {
    ...
    #-------------------------------------------------------------------------
    # original and replaces procedures ...
    #
proc vectormath::addVector_org {v1 v2 {scaling 1}} {
        #
    foreach {x1 y1} $v1 {x2 y2} $v2 break
    return [list [expr {$x1 + $scaling*$x2}] [expr {$y1 + $scaling*$y2}]]
}	 
proc vectormath::subVector_org {v1 v2} { return [addVector $v1 $v2 -1] }
