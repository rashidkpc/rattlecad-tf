...
###  20150411: 0.18
###            flatten_nestedList  handle list in list with llength = 1
###  20150411: 0.19
###            pdict2file
###  20150411: 0.20
###            pdict2text
...
namespace eval appUtil { 
    ...
    proc appUtil::flatten_deepList { args } {
        set argList $args
        if {[llength $args] == 0 } { return ""}
        while {[llength $argList] == 1} {
            set newList [lindex $argList 0]
            if {$newList == $argList} break
            set argList $newList
        }
        set flatList {}
        foreach e [list {*}$argList] {
            foreach ee $e { lappend flatList $ee }
        }
        return $flatList
    }
    
    proc appUtil::flatten_nestedList_debug { args } {
        set argList $args
        if {[llength $args] == 0 } { return ""}
            puts "            -> $argList   <- [llength $argList]"
            set x 0
        while {[llength $argList] == 1} {
            set newList [lindex $argList 0]
                incr x
                puts "            -> $x - $newList   <- [llength $newList]"
            if {$newList == $argList} break
            set argList $newList
        }
            puts "    <D> -- $argList"
